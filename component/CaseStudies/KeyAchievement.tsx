  ...
  return (
    <div className="container py-5 lg:py-10">
      <div className="flex flex-col items-center mb-5 lg:mb-10">
        <h4 className="text-[#6e51e0] text-xs text-center font-medium mb-3">{keyAchievement?.subtitle}</h4>
      ...
      <div>
        {keyAchievement?.achievments?.map((achievement, index) => (
          <div
            key={index}
            className={`flex  justify-center gap-3 lg:gap-8 first:mb-8 ${
            ...
            <div className="p-1 bg-white rounded-xl shadow-[0px_8px_40px_0px_rgba(6,11,19,0.04)] overflow-hidden">
              <div className="p-5 h-[300px] lg:h-full lg:p-10 rounded-xl border-[.5px]">
                <div className="flex gap-4 mb-3 lg:mb-5">
                  <i
                    className={`text-xl text-[#6E51E0] border p-2 rounded-full fa-solid ${achievement?.icon}`}
                  ></i>
                  <span className="text-[#060b13] text-xl font-semibold">{achievement?.name}</span>
                </div>
