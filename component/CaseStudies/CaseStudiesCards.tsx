...
const CaseStudiesCards: React.FC<CaseStudyProps> = ({ caseStudyData }) => {
  ...
  return (
    <div className='bg-whiterounded-t-[60px] py-10'>
        ...
        <div className="py-20 ">
          ...
          <div className="flex flex-col gap-10 ">
            {caseStudyData.map((caseStudy) => (
              <div
                key={caseStudy.id}
                className={`flex ${caseStudy.id % 2 === 1 ? ' flex-col lg:flex-row ' : ' flex-col-reverse lg:flex-row-reverse'} gap-10 justify-between`}
              >
                {/* Left Card (Image + Title + Tags + Description) */}
                <div className="flex flex-col gap-2" data-aos={caseStudy.id % 2 === 1 ? 'fade-up-left' : 'fade-up-right'}>
                  ...
                  <p className="text-lg font-light text-gray-400">{caseStudy.description}</p>
                  <div className="flex flex-row gap-3 ">
                      {caseStudy.tags.map((tag, index) => (
                        <div
                          key={index}
                          className={`flex justify-center items-center text-sm px-2 rounded-3xl text-white ${getFixedColor(index)}`}
                        >
                ...
                <div className="flex flex-col gap-2 mt-32" data-aos={caseStudy.id % 2 === 1 ? 'fade-up-right' : 'fade-up-left'}>
                  <div className="overflow-hidden">
                    <img
                      className="w-[600px] h-[500px] object-cover transform transition-transform duration-300 ease-in-out hover:scale-110 hover:transform-origin-center"
                      src={caseStudy.image}
